            ...
            async for html in html_source.fetch():
                try:
                    ...
                    if error_handling == "raise":
                        error_message = f"Run into an error processing article r!{html.requested_url}"
                        ...
                        raise ValueError(f"Unknown value r!{error_handling} for parameter <error_handling>")

                if extraction_filter and extraction_filter(extraction):
                    basic_logger.debug(f"Skipped article at r!{html.requested_url} because of extraction filter")
                    yield None
                else:
